...
class AnnotationDialogue(wx.Dialog):
    ...
    def __init__(self, *args, **kw):
        ...
        self.text_ctrl = wx.TextCtrl(self, -1, style=wx.TE_MULTILINE, size=(WorldState.Instance().dispW/6, WorldState.Instance().dispH/8))
        ...
        self.start_time = wx.SpinCtrl(self, max=1000000, size=(120, -1), initial=int(WorldState.Instance().session_dict['clock']), value=str(int(WorldState.Instance().session_dict['clock'])))
        ...
        self.end_time = wx.SpinCtrl(self, max=1000000, size=(120, -1), initial=int(WorldState.Instance().session_dict['clock'] + WorldState.Instance().session_dict['clock_increment']*100), value=str(int(WorldState.Instance().session_dict['clock'] + WorldState.Instance().session_dict['clock_increment']*100)))
        ...
        instruction_text.Wrap(WorldState.Instance().dispW/4)
    ...
    def calculate_duration(self, e):
        duration = int((self.end_time.GetValue() - self.start_time.GetValue()) * WorldState.Instance().session_dict['clock_increment'])
    ...
    def on_ok(self, e):
        ...
        WorldState.Instance().temp_anime_annotation = AnimationAnnotation(text, start, finish, WorldState.Instance().session_dict['cur_annotation_id'])

        self.Close()

    def on_cancel(self, e):
        WorldState.Instance().temp_anime_annotation = None
        self.Close()
